...
class CgEventAddPage extends StatefulWidget {
  CgEventAddPage(this.auth, this.chugoku, this.event);
...
class _EventAddPageState extends State<CgEventAddPage> {
  ...
  void initState() {
    ...
    if (widget.event.data != null && widget.event.data.poleSearch == true) {
      _addData.eventPole = <String, dynamic>{
        'line' : widget.chugoku.line.name,
        'pole': widget.chugoku.pole.name,
        'located_at' : GeoPoint(widget.chugoku.pole.latitude, widget.chugoku.pole.longitude)
      };
      widget.event.data = EventData();
      _poleController.text = '${_addData.eventPole['line']} / ${_addData.eventPole['pole']}';
    }
  ...
  Future<void> _upload() async{
    if (await showPopupLocationService(context) != false) {
      ...
      try {
        ...
        setEvent(_user, widget.chugoku, _addData);
