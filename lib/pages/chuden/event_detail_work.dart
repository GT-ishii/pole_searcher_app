...
class ChEventDetailWorkPage extends StatefulWidget {
  ChEventDetailWorkPage(this.auth, this.chuden, this.event);
  ...
  @override
  _ChEventDetailWorkPageState createState() => _ChEventDetailWorkPageState();
}

class _ChEventDetailWorkPageState extends State<ChEventDetailWorkPage> {
  ...
  void initState() {
    ...
    setBeforePage(belongToCh, ChNavigationMenuItem.event.index, ChEventDetailWorkPath);
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      onWillPop: () async {
        Navigator.popUntil(context, ModalRoute.withName(ChEventListWorkPath));
        setBeforePage(5, ChNavigationMenuItem.event.index, ChEventListWorkPath);
      ...
      list.add(showCheckBox(statusList[i], _status[i], (value){
        setState(() {
          if (_oldIndex != i) {
            ...
            _status[i] = true;
            updateEventStatus(_user, widget.chuden, widget.event.data.id, statusList[i]);
  ...
  void _cbDelete(){
    ...
    showPopupConfirm(
        ...
        (){
          ...
          deleteEvent(_user, widget.chuden, widget.event.data.id);
  ...
  void _cbWork(){
    ...
    setEventWorker(_user, widget.chuden, widget.event.data.id);
