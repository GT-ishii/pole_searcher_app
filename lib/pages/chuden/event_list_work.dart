...
class ChEventListWorkPage extends StatefulWidget {
  ChEventListWorkPage(this.auth, this.chuden, this.event);
  ...
  @override
  _ChEventListWorkPageState createState() => _ChEventListWorkPageState();
}

class _ChEventListWorkPageState extends State<ChEventListWorkPage> {
  ...
  void initState() {
    ...
    setBeforePage(belongToCh, ChNavigationMenuItem.event.index, ChEventListWorkPath);
    ...
    widget.auth.getUser()
        .then((user) {
          ...
          getEventWorks(_user, widget.chuden, widget.event.setting.notNotificationStatus).then((eventList) {
            setState(() {
  ...
  Future<void> _refleshList() async{
    ...
    await getEventWorks(_user, widget.chuden, widget.event.setting.notNotificationStatus).then((eventlist) {
      setState(() {
        _eventList = List.from(eventlist);
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      onWillPop: () async {
        ...
        setBeforePage(5, ChNavigationMenuItem.event.index, ChEventSelectPath);
  ...
  Future<void> _goNext(int index) async{
    ...
    await Navigator.pushNamed(context, ChEventDetailWorkPath)
        .whenComplete(() async{
          showProgressIndicator(context);
          await _refleshList();
          Navigator.pop(context);
