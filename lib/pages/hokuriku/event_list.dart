...
class HrEventListPage extends StatefulWidget {
  HrEventListPage(this.auth, this.hokuriku, this.event);
  ...
  @override
  _HrEventListPageState createState() => _HrEventListPageState();
}

class _HrEventListPageState extends State<HrEventListPage> {
  ...
  void initState() {
    ...
    widget.auth.getUser()
        .then((user) {
          ...
          getEvents(_user, widget.hokuriku,HokurikuID).then((eventList) {
            setState(() {
  ...
  Future<void> _refleshList() async{
    ...
    await getEvents(_user, widget.hokuriku, HokurikuID).then((eventlist) {
      setState(() {
        _eventList = List.from(eventlist);
        _searchList = List.from(eventlist);
      });
  ...
  Future<void> _goNext(int index) async{
    ...
    await Navigator.pushNamed(context, HrEventDetailPath)
        .whenComplete(() async{
          showProgressIndicator(context);
          await _refleshList()
