...
class HrEditProfilePage extends StatefulWidget {
  HrEditProfilePage(this.auth, this.hokuriku);

  final Auth auth;
  final Hokuriku hokuriku;

  @override
  _HrEditProfilePageState createState() => _HrEditProfilePageState();
}
...
class _HrEditProfilePageState extends State<HrEditProfilePage> {
  ...
  void initState() {
    ...
    setBeforePage(
        belongToHr, HrNavigationMenuItem.setting.index, HrEditProfilePath);
            ...
            children: <Widget>[
              ...
              showSubmit(
                _formKey, '変更', const Size(210,50), 22, Colors.indigo,
                  _canSubmit ? _hrPw : null),
            ],
          ),
        )
      ),
      bottomNavigationBar: hrBottomNavigationBar(
        context, widget.auth, HrNavigationMenuItem.setting.index, belongToHr, widget.hokuriku)
    );
  }
